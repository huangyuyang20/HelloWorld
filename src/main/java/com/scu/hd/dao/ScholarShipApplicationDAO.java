...
public interface ScholarShipApplicationDAO {
    ...
    @Insert("insert into scholarship_application(scholarship_id,stu_id,scho_personalstmt," +
            "scho_approvetime,scho_submittime,scho_approve) " +
            "values(#{scholarshipId},#{stuId},#{schoPersonalstmt}," +
            "#{schoApprovetime},#{schoSubmittime},#{schoApprove})")
    ...
    //所有申请信息
    @Select("select sa.scholarship_id as scholarshipId,sa.stu_id as stuId,sa.scho_approve as schoApprove," +
            "sa.scho_approvetime as schoApprovetime,sa.scho_personalstmt as schoPersonnalStmt,ss.scho_name as schoName,ss.scho_time as schoTime " +
            "from scholarship_application as sa,scholarship as ss where " +
            "sa.scholarship_id = ss.scholarship_id and sa.scho_approve = 0")
